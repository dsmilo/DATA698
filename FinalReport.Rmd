---
title: "EO88 Energy Analytics & Data Cleansing"
...
bsny_agency %>%
  ...
  labs(title = "Distribution of agency EUI reduction",
       subtitle = "SFY 2016-17 vs. SFY 2010-11 baseline",
       y = "Number of agencies", caption = "Excluding agency with 500% increase and 2 yet-to-report agencies")
```
...
```{r facility-state-plot}
bsny_facility %>% 
  ggplot() +
  geom_line(aes(x = SFY, y = Reduction, group = ESPLocationID), lwd = 0.25, alpha = 0.1) +
  geom_point(data = bsny_state, aes(x = SFY, y = Reduction),
             size = 2, col = "black") +
  geom_line(data = bsny_state, aes(x = SFY, y = Reduction), group = 1,
            lwd = 1, col = "black") +
  geom_hline(yintercept = -0.2, lty = 2, lwd = 1, col = "#006ba6") +
  geom_hline(yintercept = 0, lty = 3, col = "black") +
  scale_y_continuous(labels = scales::percent, limits = c(-0.6, 0.6)) +
...
fuel_viz <- eo88_final %>%
  ...
  scale_y_continuous("Change in EUI", limits = c(NA, 1), labels = scales::percent) +
  ggtitle("EUI performance by fuel")
...
type_viz <- eo88_final %>%
  ...
  geom_line(show.legend = FALSE) +
  scale_y_continuous("Change in EUI", labels = scales::percent) +
  ggtitle("EUI performance by primary building type")

gridExtra::grid.arrange(fuel_viz, type_viz, ncol = 1)
```
...
![Tableau View Screenshots](data/tableau-screenshots.png)
