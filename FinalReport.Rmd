...
Each of these patterns was investigated individually, as the approach to imputing the missing values was expcted to vary.
...
### Data Imputation
All remaining missing values have been flagged for imputation with random forest (25 values) or linear regression (672 values) models.  Each imputation method is implemented separately using the `mice` package.  The `AccountNumber` field is converted to a factor and included in the impuation predictor matrix -- this leads to the creation of an individual fit for each account.
...
```{r state-eui-table}
bsny_state %>%
  mutate(Reduction = paste0(round(Reduction * 100, 2), "%")) %>%
  pander()
```
...
```{r agency-final-plot}
bsny_agency %>%
  filter(SFY == "2016-17", Reduction < 5) %>%
  ggplot(aes(x = Reduction)) +
  geom_histogram(binwidth = 0.05, alpha = 0.5, col = "black") +
  scale_x_continuous(labels = scales::percent) +
  geom_vline(xintercept = -0.2, lty = 2, lwd = 1, col = "#006ba6") +
  geom_vline(xintercept = 0, lty = 3, lwd = 0.5) +
  labs()
```
...
Gergely Dar?czi and Roman Tsegelskyi (2017). pander: An R 'Pandoc' Writer. R package version 0.6.1. https://CRAN.R-project.org/package=pander
