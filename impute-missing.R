...
# viz missing
VIM::aggr(imp_data)
VIM::aggr(imp_data %>% select(Use, Demand, Cost), numbers = TRUE, prop = FALSE, bars = FALSE)
...
### flag others for imputation -- assume random forest
imp_data <- imp_data %>%
...
### handles 419/503 accts (83%) & 8341/9278 (90%) of no_use -- REPLACE WITH ZERO
imp_data <- imp_data %>% 
  mutate(Imputed = if_else(AccountNumber %in% cost_only$AccountNumber, "zero", Imputed),
...
no_use %>% 
  filter(MonthDiff == 0) %>% 
  distinct(AccountNumber, ESPLocationID, Fuel, Month) %>% nrow()
...
imp_data %>% 
  ...
  facet_wrap(~ AccountNumber, scales = "free", ncol = 5, nrow = 1, dir = "v") +
  scale_y_continuous(trans = "log10")

### 4 of 5 show NAs around zero or in last months
### likely cancelled accts with remaining charges -- zero out
### impute remaining with regression
no_use_elec <- no_use_elec %>%
  filter(AccountNumber != "Electric-40-NORRWF-0126-E-001--INTEGRYS-N01000005531926")
...
no_use_fuel %>% 
  group_by(AccountNumber) %>% 
  summarize(shortest = min(MonthDiff, na.rm = TRUE),
...
imp_data %>% 
  ...
  facet_wrap(~ AccountNumber, scales = "free", ncol = 2) +
...
# impute separately by account
rf_data <- split(rf_data, rf_data$AccountNumber)
rf_imp  <- lst() # container

# function to impute -- necessary for "try" statement
imp_rf <- function(n, method) {
  # prepare imputation method & predictors
  rf_init <- mice(rf_data[[rf_accts[n]]], maxit = 0, seed = 100)
  rf_meth <- rf_init$method
  rf_mat  <- rf_init$predictorMatrix
  
  # do not use share as predictor
  rf_mat[, "Share"] <- 0
  # only impute use
  rf_meth[names(rf_meth) != "Use"] <- ""
  # set methods
  rf_meth["Use"] <- method
  
  # impute
  rf_imp_tmp <- mice(rf_data[[rf_accts[n]]],
                      method = rf_meth, predictorMatrix = rf_mat,
                      m = 5, printFlag = FALSE, seed = 100)
  # gather imputed set
  rf_imp[[rf_accts[n]]] <<- complete(rf_imp_tmp)
}

# try imputation for each acct needing rf
for (a in 1:length(rf_accts)) {
  try(imp_rf(a, "rf"))
}

# re-gather
rf_data <- bind_rows(rf_data) %>% 
  arrange(AccountNumber, Year, Month) %>% 
  select(-(FloorArea:TMAX))
rf_imp  <- bind_rows(rf_imp) %>% 
  arrange(AccountNumber, Year, Month) %>% 
  select(-(FloorArea:TMAX))
...
# impute separately by account
reg_data <- split(reg_data, reg_data$AccountNumber)
reg_imp  <- lst() # container

# function to impute -- necessary for "try" statement
imp_reg <- function(n, method) {
  # prepare imputation method & predictors
  reg_init <- mice(reg_data[[reg_accts[n]]], maxit = 0, seed = 100)
  reg_meth <- reg_init$method
  reg_mat  <- reg_init$predictorMatrix
  
  # only use cost as predictor
  reg_mat[, which(colnames(reg_mat) != "Cost")] <- 0
  # only impute use
  reg_meth[names(reg_meth) != "Use"] <- ""
  # set methods
  reg_meth["Use"] <- method
  
  # impute
  reg_imp_tmp <- mice(reg_data[[reg_accts[n]]],
                      method = reg_meth, predictorMatrix = reg_mat,
                      m = 5, printFlag = FALSE, seed = 100)
  # gather imputed set
  reg_imp[[reg_accts[n]]] <<- complete(reg_imp_tmp)
}

# try imputation for each acct needing reg
for (b in 1:length(reg_accts)) {
  try(imp_reg(b, "norm"))
}

# re-bind & arrange
reg_data <- bind_rows(reg_data) %>% 
  arrange(AccountNumber, Year, Month)
reg_imp  <- bind_rows(reg_imp) %>% 
  arrange(AccountNumber, Year, Month)

# check consistency -- all equal except nas (don't show in table)
...
rf_data %>% 
  ...
  geom_point() +
  geom_line() +
  geom_point(data = imputed[["rf"]], aes(x = Month, y = Use), col = "red") +
  facet_wrap(~ AccountNumber, scales = "free")
...
reg_data %>% 
  ...
  geom_point(data = filter(imputed[["reg"]], AccountNumber %in% reg_accts[1:4]),
...
eo88 <- imp_data %>% 
  left_join(imputed[["rf"]], by = c("AccountNumber", "Month", "Share"), 
            suffix = c("", "_rf")) %>% 
  mutate(Use = if_else(Imputed == "rf", Use_rf, Use)) %>% 
  left_join(imputed[["reg"]], by = c("AccountNumber", "Month", "Share", "Cost"),
            suffix = c("", "_reg")) %>% 
  mutate(Use = if_else(Imputed == "reg", Use_reg, Use)) %>% 
  select(-Use_rf, -Use_reg, -Status)

# write to db
con <- dbConnect(MySQL(), default.file = paste0(getwd(), "/", ".my.cnf"))
db <- dbSendQuery(con, "USE EO88;")
dbFetch(db)
dbWriteTable(con, "consumption_filingdata_imputed", eo88, row.names = FALSE)
...
dbWriteTable(con, "consumption_filingdata_summed", eo88, row.names = FALSE)
dbDisconnect(con)
